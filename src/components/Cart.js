        ...
        <div className='p-4 lg:p-6 grid gap-4'>
          {cart.map(({ id, name, newPrice, quantity, image }) => {
            return (
              <article
                key={id}
                className='grid grid-cols-[auto_1fr_auto] gap-x-4'
              >
          ...
          <button className='hover:opacity-90 transition-all block w-full font-bold text-blue-100 text-center bg-orange-200 rounded-md px-[1em] py-[.5em]'>
            Checkout
          </button>
        </div>
      ) : (
